      ...
      if (good > 0) {
        ...
        if (branch) {
          await tx.productBranchStock.upsert({
            ...
            create: { productId: item.productId, branchId: branch.id, quantityInStock: good },
          });
        ...
        await tx.stockMovement.create({
          data: { productId: item.productId, branchId: branch?.id, type: "purchase", quantity: good, notes: `PO ${order.poNumber}`, userId: user.id },
        });
      }
    }
